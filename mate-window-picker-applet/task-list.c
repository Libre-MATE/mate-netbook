...
static void task_list_set_show_all_windows(TaskList *list,
                                           gboolean show_all_windows) {
...
static void on_task_item_closed(TaskItem *item, TaskList *list) {
...
static void on_window_opened(WnckScreen *screen, WnckWindow *window,
  ...
  if (type == WNCK_WINDOW_DESKTOP || type == WNCK_WINDOW_DOCK ||
      type == WNCK_WINDOW_SPLASHSCREEN || type == WNCK_WINDOW_MENU)
    return;
...
static void task_list_get_property(GObject *object, guint prop_id,
...
static void task_list_set_property(GObject *object, guint prop_id,
...
static void task_list_class_init(TaskListClass *klass) {
  ...
  g_object_class_install_property(
      obj_class, PROP_SHOW_ALL_WINDOWS,
      g_param_spec_boolean("show_all_windows", "Show All Windows",
                           "Show windows from all workspaces", TRUE,
...

{
  ...
  list = g_object_new(TASK_TYPE_LIST, "homogeneous", FALSE, "spacing", 0, NULL);
