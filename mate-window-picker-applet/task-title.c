...
static gboolean start_logout_dialog(TaskTitle *title) {
  ...
  if (!error) {
    ...
    g_app_info_launch(app_info, NULL, G_APP_LAUNCH_CONTEXT(launch_context),
                      &error);
  ...
  dialog = gtk_message_dialog_new(
      ...
      _("There was an error executing '%s': %s"), LOGOUT, error->message);
...
static gboolean on_close_clicked(GtkButton *button, GdkEventButton *event,
                                 TaskTitle *title) {
...
static gboolean on_button_draw(GtkWidget *widget, cairo_t *cr,
  ...
  if (priv->mouse_in_close_button) {
    ...
    gtk_paint_box(style, cr, GTK_STATE_PRELIGHT, GTK_SHADOW_NONE, NULL, NULL,
                  area.x, area.y + 2, area.width, area.height - 4);
  }
  return FALSE;
}
...
static void on_name_changed(WnckWindow *window, TaskTitle *title) {
  ...
  priv = title->priv;
  if (priv->window != window) return;

  gtk_label_set_text(GTK_LABEL(title->priv->label),
                     wnck_window_get_name(window));
  gtk_widget_set_tooltip_text(GTK_WIDGET(title), wnck_window_get_name(window));
  gtk_widget_queue_draw(GTK_WIDGET(title));
}
...
static void on_state_changed(WnckWindow *window, WnckWindowState changed_mask,
                             WnckWindowState new_state, TaskTitle *title) {
  TaskTitlePrivate *priv;
...
static void on_active_window_changed(WnckScreen *screen, WnckWindow *old_window,
                                     TaskTitle *title) {
  TaskTitlePrivate *priv;
  ...
  if (act_window) type = wnck_window_get_window_type(act_window);

  if (WNCK_IS_WINDOW(act_window) && wnck_window_is_skip_tasklist(act_window) &&
  ...
  if (!WNCK_IS_WINDOW(act_window) ||
    ...
    if (priv->show_home_title) {
      ...
      gtk_image_set_from_pixbuf(GTK_IMAGE(priv->button_image), priv->quit_icon);
    ...
    gtk_image_set_from_icon_name(GTK_IMAGE(priv->button_image), "window-close",
                                 GTK_ICON_SIZE_MENU);
    ...
    g_signal_connect(act_window, "name-changed", G_CALLBACK(on_name_changed),
                     title);
    g_signal_connect(act_window, "icon-changed", G_CALLBACK(on_icon_changed),
                     title);
...
static gboolean on_draw(GtkWidget *w, cairo_t *cr) {
  if (gtk_widget_get_state_flags(w) == GTK_STATE_FLAG_ACTIVE) {
    ...
    gtk_render_frame(context, cr, 0, 0, gtk_widget_get_allocated_width(w),
                     gtk_widget_get_allocated_height(w));
  }

  gtk_container_propagate_draw(GTK_CONTAINER(w), gtk_bin_get_child(GTK_BIN(w)),
                               cr);
  return TRUE;
}

/* GObject stuff */
static void task_title_set_show_home_title(TaskTitle *title,
                                           gboolean show_home_title) {
  TaskTitlePrivate *priv = title->priv;
...
static void task_title_set_bold_window_title(TaskTitle *title,
                                             gboolean bold_window_title) {
  TaskTitlePrivate *priv = title->priv;
...
static void task_title_get_property(GObject *object, guint prop_id,
...
static void task_title_set_property(GObject *object, guint prop_id,
...
static void task_title_class_init(TaskTitleClass *klass) {
  ...
  g_object_class_install_property(
      obj_class, PROP_SHOW_HOME_TITLE,
      g_param_spec_boolean("show_home_title", "Show Home Title",
                           "Show Home Title", FALSE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      obj_class, PROP_BOLD_WINDOW_TITLE,
      g_param_spec_boolean("bold_window_title", "Bold Window Title",
                           "Show windows title with a bold face", TRUE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
...
static void task_title_init(TaskTitle *title) {
  ...
  gtk_alignment_set_padding(GTK_ALIGNMENT(priv->align), 0, 0, 6, 6);
  ...
  priv->button = g_object_new(GTK_TYPE_EVENT_BOX, "visible-window", FALSE,
                              "above-child", TRUE, NULL);
  ...
  g_signal_connect(priv->button, "draw", G_CALLBACK(on_button_draw), title);
  ...
  priv->quit_icon =
      gtk_icon_theme_load_icon(theme, "system-log-out", width, 0, NULL);
  ...
  g_signal_connect(title, "button-press-event", G_CALLBACK(on_button_release),
                   NULL);
}
...

{
  ...
  title = g_object_new(TASK_TYPE_TITLE, "border-width", 0, "name",
                       "tasklist-button", "visible-window", FALSE, NULL);
