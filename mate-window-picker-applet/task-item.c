...
enum { TASK_ITEM_CLOSED_SIGNAL, LAST_SIGNAL };
...
static void update_hints(TaskItem *item) {
  ...
  wnck_window_set_icon_geometry(window, x, y, allocation_widget.width,
                                allocation_widget.height);
}

static gboolean on_task_item_button_released(GtkWidget *widget,
...
static void task_item_set_visibility(TaskItem *item) {
  ...
  gboolean show_all =
      task_list_get_show_all_windows(TASK_LIST(task_list_get_default()));
...
static void task_item_get_preferred_width(GtkWidget *widget,
                                          gint *minimal_width,
                                          gint *natural_width) {
  *minimal_width = *natural_width = DEFAULT_TASK_ITEM_WIDTH;
}

static void task_item_get_preferred_height(GtkWidget *widget,
                                           gint *minimal_height,
                                           gint *natural_height) {
  *minimal_height = *natural_height = DEFAULT_TASK_ITEM_HEIGHT;
}

static GdkPixbuf *task_item_sized_pixbuf_for_window(TaskItem *item,
                                                    WnckWindow *window,
                                                    gint size) {
  GdkPixbuf *pbuf = NULL;
  ...
  if (wnck_window_has_icon_name(window)) {
    ...
    if (gtk_icon_theme_has_icon(icon_theme, icon_name)) {
      GdkPixbuf *internal = gtk_icon_theme_load_icon(
          icon_theme, icon_name, size, GTK_ICON_LOOKUP_FORCE_SIZE, NULL);
  ...
  if (MAX(width, height) != size) {
    ...
    GdkPixbuf *tmp = pbuf;
    pbuf = gdk_pixbuf_scale_simple(tmp, (gint)(width * scale),
                                   (gint)(height * scale), GDK_INTERP_HYPER);
...
static gboolean task_item_draw(GtkWidget *widget, cairo_t *unused) {
  cairo_t *cr;
  TaskItem *item;
  GdkRectangle area;
  TaskItemPrivate *priv;
  ...
  if (GDK_IS_PIXBUF(pbuf) && gdk_pixbuf_get_width(pbuf) != size &&
  ...
  if (active) {
    cairo_rectangle(cr, area.x + 1, area.y - 1, area.width - 2,
                    area.height - 2);
    ...
    glow_pattern = cairo_pattern_create_radial(area.x + glow_x, area.y + glow_y,
                                               glow_x * 0.3, area.x + glow_x,
                                               area.y + glow_y, glow_x * 1.4);
  ...
  if (!pbuf) {
    pbuf = priv->pixbuf =
        task_item_sized_pixbuf_for_window(item, priv->window, size);
  }

  if (active || priv->mouse_over || attention) {
    gdk_cairo_set_source_pixbuf(
        cr, pbuf, (area.x + (area.width - gdk_pixbuf_get_width(pbuf)) / 2),
        (area.y + (area.height - gdk_pixbuf_get_height(pbuf)) / 2));
  } else {
    GdkPixbuf *desat = gdk_pixbuf_new(
        GDK_COLORSPACE_RGB, TRUE, gdk_pixbuf_get_bits_per_sample(pbuf),
        gdk_pixbuf_get_width(pbuf), gdk_pixbuf_get_height(pbuf));

    if (desat) {
      gdk_pixbuf_saturate_and_pixelate(pbuf, desat, 0, FALSE);
    } else /* just paint the colored version as a fallback */
    {
      desat = g_object_ref(pbuf);
    }
    gdk_cairo_set_source_pixbuf(
        cr, desat, (area.x + (area.width - gdk_pixbuf_get_width(desat)) / 2),
...
static gboolean on_button_pressed(GtkWidget *button, GdkEventButton *event,
                                  TaskItem *item) {
  WnckWindow *window;
...
static gboolean on_enter_notify(GtkWidget *widget, GdkEventCrossing *event,
                                TaskItem *item) {
  g_return_val_if_fail(TASK_IS_ITEM(item), FALSE);
...
static void on_window_state_changed(WnckWindow *window,
                                    ...
                                    WnckWindowState new_state, TaskItem *item) {
...
static void on_window_workspace_changed(WnckWindow *window, TaskItem *item) {
  g_return_if_fail(TASK_IS_ITEM(item));
...
static void on_screen_active_viewport_changed(WnckScreen *screen,
                                              TaskItem *item) {
  g_return_if_fail(TASK_IS_ITEM(item));
...
static void on_screen_window_closed(WnckScreen *screen, WnckWindow *window,
                                    TaskItem *item) {
  TaskItemPrivate *priv;
  ...
  if (priv->window == window) {
    g_signal_handlers_disconnect_by_func(
        screen, G_CALLBACK(on_screen_active_viewport_changed), item);
    g_signal_handlers_disconnect_by_func(
        screen, G_CALLBACK(on_screen_active_window_changed), item);
    g_signal_handlers_disconnect_by_func(
        screen, G_CALLBACK(on_screen_active_workspace_changed), item);
    g_signal_handlers_disconnect_by_func(
        screen, G_CALLBACK(on_screen_window_closed), item);
    g_signal_handlers_disconnect_by_func(
        window, G_CALLBACK(on_window_workspace_changed), item);
    g_signal_handlers_disconnect_by_func(
        window, G_CALLBACK(on_window_state_changed), item);
    g_signal_handlers_disconnect_by_func(
        window, G_CALLBACK(on_window_icon_changed), item);

    g_signal_emit(G_OBJECT(item), task_item_signals[TASK_ITEM_CLOSED_SIGNAL],
                  0);
  }
}
...
static gboolean activate_window(GtkWidget *widget) {
  ...
  if (active) {
    ...
    window = priv->window;
    if (WNCK_IS_WINDOW(window)) wnck_window_activate(window, time(NULL));
  }
...
static void on_drag_leave(GtkWidget *item, GdkDragContext *context,
                          guint time) {
  g_object_set_data(G_OBJECT(item), "drag-true", GINT_TO_POINTER(0));
}

static gboolean on_drag_motion(GtkWidget *item, GdkDragContext *context, gint x,
                               gint y, guint t) {
  gint active;
...
static void task_item_dispose(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      item->priv->screen, G_CALLBACK(on_screen_active_viewport_changed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->screen, G_CALLBACK(on_screen_active_window_changed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->screen, G_CALLBACK(on_screen_active_workspace_changed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->screen, G_CALLBACK(on_screen_window_closed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->window, G_CALLBACK(on_window_workspace_changed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->window, G_CALLBACK(on_window_state_changed), item);
  g_signal_handlers_disconnect_by_func(
      item->priv->window, G_CALLBACK(on_window_icon_changed), item);
...
static void task_item_class_init(TaskItemClass *klass) {
  ...
  task_item_signals[TASK_ITEM_CLOSED_SIGNAL] =
      g_signal_new("task-item-closed", G_TYPE_FROM_CLASS(klass),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(TaskItemClass, itemclosed), NULL, NULL,
...
GtkWidget *task_item_new(WnckWindow *window) {
  ...
  item = g_object_new(TASK_TYPE_ITEM, "has-tooltip", TRUE, "visible-window",
                      FALSE, "above-child", TRUE, NULL);
  ...
  gtk_drag_dest_set(item, GTK_DEST_DEFAULT_MOTION | GTK_DEST_DEFAULT_DROP,
                    drop_types, n_drop_types, GDK_ACTION_COPY);
  gtk_drag_dest_add_uri_targets(item);
  gtk_drag_dest_add_text_targets(item);
  g_signal_connect(item, "drag-motion", G_CALLBACK(on_drag_motion), NULL);
  g_signal_connect(item, "drag-leave", G_CALLBACK(on_drag_leave), NULL);
  ...
  g_signal_connect(screen, "window-closed", G_CALLBACK(on_screen_window_closed),
                   item);
  ...
  g_signal_connect(window, "state-changed", G_CALLBACK(on_window_state_changed),
                   item);
  g_signal_connect(window, "icon-changed", G_CALLBACK(on_window_icon_changed),
                   item);

  g_signal_connect(item, "button-release-event",
                   G_CALLBACK(on_task_item_button_released), item);
  g_signal_connect(item, "button-press-event", G_CALLBACK(on_button_pressed),
                   item);
  g_signal_connect(item, "size-allocate", G_CALLBACK(on_size_allocate), item);
  g_signal_connect(item, "query-tooltip", G_CALLBACK(on_query_tooltip), item);
  g_signal_connect(item, "enter-notify-event", G_CALLBACK(on_enter_notify),
                   item);
  g_signal_connect(item, "leave-notify-event", G_CALLBACK(on_leave_notify),
                   item);
  g_signal_connect(item, "drag-motion", G_CALLBACK(on_drag_motion), item);
  g_signal_connect(item, "drag-leave", G_CALLBACK(on_drag_leave), item);
