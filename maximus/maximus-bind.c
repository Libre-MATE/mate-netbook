...
static const gchar *get_fullscreen_keystroke(GList *rules, WnckWindow *window) {
...
static const gchar *get_unfullscreen_keystroke(GList *rules,
                                               WnckWindow *window) {
  WnckClassGroup *group;
  const gchar *class_name;
  GList *r;
...
static gboolean real_fullscreen(MaximusBind *bind) {
  ...
  if (keystroke) {
    ...
    if (egg_accelerator_parse_virtual(keystroke, &keysym, &modifiers)) {
      ...
      if (modifiers & EGG_VIRTUAL_SHIFT_MASK) mods |= FAKEKEYMOD_SHIFT;
      if (modifiers & EGG_VIRTUAL_CONTROL_MASK) mods |= FAKEKEYMOD_CONTROL;
      if (modifiers & EGG_VIRTUAL_ALT_MASK) mods |= FAKEKEYMOD_ALT;
      if (modifiers & EGG_VIRTUAL_META_MASK) mods |= FAKEKEYMOD_META;

      g_debug("Sending fullscreen special event: %s = %d %d", keystroke, keysym,
              mods);
...
static void fullscreen(MaximusBind *bind, WnckWindow *window) {
...
static gboolean real_unfullscreen(MaximusBind *bind) {
  MaximusBindPrivate *priv;
  GdkDisplay UNUSED_VARIABLE *display;
  WnckWindow *active;
  const gchar *keystroke;
...
static void unfullscreen(MaximusBind *bind, WnckWindow *window) {
  MaximusBindPrivate UNUSED_VARIABLE *priv;
...
static void on_binding_activated(gchar *keystring, MaximusBind *bind) {
  MaximusBindPrivate *priv;
  WnckWindow *active;
  ...
  if (wnck_window_get_window_type(active) != WNCK_WINDOW_NORMAL) return;
...
/* Callbacks */
static gboolean binding_is_valid(const gchar *binding) {
...
static void on_binding_changed(GSettings *settings, gchar *key,
                               MaximusBind *bind) {
  MaximusBindPrivate *priv;
  ...
  if (binding_is_valid(priv->binding))
    tomboy_keybinder_bind(priv->binding,
                          (TomboyBindkeyHandler)on_binding_activated, bind);

  g_print("Binding changed: %s\n", priv->binding);
}

/* GObject stuff */
static void create_rule(MaximusBind *bind, const gchar *filename) {
...
#define RULE_UNFULLSCREEN "Unfullscreen"
  ...
  rule->wm_class = g_key_file_get_string(file, RULE_GROUP, RULE_WMCLASS, NULL);
  rule->fullscreen =
      g_key_file_get_string(file, RULE_GROUP, RULE_FULLSCREEN, NULL);
  rule->unfullscreen =
      g_key_file_get_string(file, RULE_GROUP, RULE_UNFULLSCREEN, NULL);
...
static void load_rules(MaximusBind *bind, const gchar *path) {
  MaximusBindPrivate UNUSED_VARIABLE *priv;
  GDir *dir;
  const gchar *name;
...
static void maximus_bind_finalize(GObject *obj) {
...
static void maximus_bind_class_init(MaximusBindClass *klass) {
...

{
  ...
  if (!bind) bind = g_object_new(MAXIMUS_TYPE_BIND, NULL);
